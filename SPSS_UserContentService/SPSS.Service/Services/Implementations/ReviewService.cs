...
using SPSS.BusinessObject.Dto.Review;
...
namespace SPSS.Service.Implementations
{
    public class ReviewService : IReviewService
    {
		...
		public ReviewService(IUnitOfWork unitOfWork, IMapper mapper)
        {
			...
			_reviewImageRepository = _unitOfWork.GetRepository<IReviewImageRepository>();
		}
        ...
        //    var totalCount = await _reviewRepository.Entities
        //        .Where(r => !r.IsDeleted)
        //        .CountAsync();

        //    // Truy vấn dữ liệu với phân trang
        //    var cartItems = await _reviewRepository.Entities
        ...
        //    var userReviews = await _reviewRepository.Entities
        //        .Include(ri => ri.ReviewImages)
        //        .Include(u => u.User)
        //        .Include(pi => pi.ProductItem)
        //            .ThenInclude(p => p.ProductConfigurations)
        ...
        public async Task<int> GetTotalReviewsByUserIdAsync(Guid userId)
        {
            ...
            var totalReviews = await _reviewRepository.Entities
                .Where(r => r.UserId == userId && !r.IsDeleted)
                .CountAsync();
        ...
        //    var query = _reviewRepository.Entities
        //        .Include(ri => ri.ReviewImages)
        //        .Include(u => u.User)
        //        .Include(pi => pi.ProductItem)
        //            .ThenInclude(p => p.ProductConfigurations)
        ...
        public async Task<ReviewForCreationDto> CreateAsync(Guid userId, ReviewForCreationDto reviewDto)
        {
            ...
            //var productReviews = await _reviewRepository.Entities
        ...
        public async Task<ReviewDto> UpdateAsync(Guid userId, ReviewForUpdateDto reviewDto, Guid id)
        {
			...
			var review = await _reviewRepository
						.GetSingleAsync(r => r.Id == id, q => q.Include(r => r.ReviewImages));
			if (review == null)
            ...
            foreach (var newImage in newImages)
            {
				...
				_reviewImageRepository.Add(newImage);
            }
